...
<<iris_example_1, cache=TRUE, size="tiny">>=
my_data = iris
my_summary = my_data %>% group_by(Species) %>% summarize(sepallen=mean(Sepal.Length),
                                                         sepalwid=mean(Sepal.Width),
                                                         petallen=mean(Petal.Length),
                                                         petalwid=mean(Petal.Width))
my_summary
@

This functionality includes graphs!
<<iris_example_2, cache=TRUE, fig.width=10, fig.asp=0.33, out.width="\\linewidth", size="tiny">>=
ggplot(my_data, aes(x=Sepal.Length, y=Sepal.Width, color=Petal.Length)) + 
...
<<airquality_example_1, cache=TRUE, fig.width=10, fig.asp=0.33, out.width="\\linewidth", warning=FALSE, echo=FALSE>>=
ggplot(airquality, aes(x=Temp, y=as.factor(Month))) + geom_joy() 
@
				\end{block}
